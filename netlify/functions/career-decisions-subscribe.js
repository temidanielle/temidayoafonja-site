...
async function isRateLimited(event, ip) {
  if (!ip) return false;
  try {
    const store = blobStore("career-decisions-rate", event);
    ...
    console.error("blobs career-decisions-rate failed, rate limiting is OFF. manual config present:", blobsAvailable(event), e);
    return false;
  }
}
...
exports.handler = async (event) => {
  ...
  if (blobsAvailable(event)) {
    try {
      const store = blobStore("career-decisions-leads", event);
      ...
      console.error("blobs career-decisions-leads write failed. manual config present:", blobsAvailable(event), e);
      recordKey = null;
    }
  } else {
