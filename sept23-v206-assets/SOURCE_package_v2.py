...
ROOT = "scratchpad/sept23/pkg/September_23_Stay_or_Leave_CANDIDATE_v2"
A = "sept23-v206-assets"
...
LAYOUT = {
 "01_LIVE_DELIVERY": [
   f"{A}/PRESENTER_VERSION_Stay_or_Leave_Live_Career_Growth_Assessment_60MIN_v2.0.6_CANDIDATE.pptx", WB],
 "02_FACILITATOR_OPERATIONS": [
   f"{A}/Capability_Formation_Free_Flagship_SOP_Sept23_2026_60MIN_v2.0.9_CANDIDATE.docx"],
 "03_QA_AND_CHANGE_CONTROL": [
   f"{A}/Free_Flagship_60MIN_v2.0.9_Change_Log_and_QA_Report.docx",
   f"{A}/Manual_Maven_and_Website_Edit_Checklist_v1.2.docx",
   f"{A}/PREVIEW_Presentation_60MIN_v2.0.6_LibreOffice.pdf",
   f"{A}/PREVIEW_SOP_60MIN_v2.0.9_LibreOffice.pdf"],
 "04_BUILD_AND_REPRODUCIBILITY": [
   f"{A}/SOURCE_build_v206.py", f"{A}/SOURCE_build_sop_v209.py",
   f"{A}/SOURCE_build_report_v209.py", f"{A}/SOURCE_build_checklist_v12.py",
   f"{A}/SOURCE_qa_v206.py"],
}
...
def main():
    ...
    man = open("scratchpad/sept23/manifest_head.txt", encoding="utf-8").read()
    ...
    Z = "September_23_Stay_or_Leave_CANDIDATE_Asset_Package_v2.zip"
    if os.path.exists(Z):
        os.remove(Z)
