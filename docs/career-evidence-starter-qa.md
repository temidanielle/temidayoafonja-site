...
| 1 | **PDF hosting.** **Closed.** The PDF is committed at `/resources/keep-the-proof-career-evidence-starter-v1-1-dac91eea9ebf.pdf`, under a filename carrying a random token. `/resources/*` carries `X-Robots-Tag: noindex, nofollow` and is disallowed in `robots.txt`. This deliberately breaks the no-PDFs convention, on operator approval. It is lead-magnet gating, not access control. | Closed |
