# -*- coding: utf-8 -*-
# ============================================================================
# EDITABLE SOURCE — Capability Position Read, Free Flagship Worksheet v1.0
#
...
#
#   python3 Capability_Position_Read_FREE_FLAGSHIP_Worksheet_v1.0_SOURCE.py
#
...
OUT = "out/Capability_Position_Read_FREE_FLAGSHIP_Worksheet_v1.0_CANDIDATE.pdf"
...
def attach_field_behaviour():
    ...
    SRC = "out/Capability_Position_Read_FREE_FLAGSHIP_Worksheet_v1.0_CANDIDATE.pdf"
    ...
    for page in w.pages:
        for a in page.get("/Annots", []):
            ...
            if INIT15.match(nm):
                ...
                aa[NameObject("/K")] = js_action(KS_15Q, nm); touched["ks15q"] += 1
            elif nm in TOTALS:
                p, lo, hi = TOTALS[nm]
                aa[NameObject("/C")] = js_action(calc_js(p, lo, hi), nm)
                o[NameObject("/Ff")] = NumberObject(1)          # read-only: it computes
                calc_order.append(a)
                touched["calc"] += 1
            if aa:
                o[NameObject("/AA")] = aa
