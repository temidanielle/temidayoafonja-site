...
MAIN = "Video_5_Main_Slides"
RECDECK = "Video_5_Reveal_Builds"
PREVIEW = "Video_5_Slide_Preview"
N = 12
...
def main():
    ...
    sheet(pngs, os.path.join(OUT, "contact-sheet.png"), 560,
          "SHOULD I MAKE AN INTERNAL MOVE? 3 QUESTIONS TO DECIDE",
          "Video 5   ·   12 slides   ·   1920 x 1080   ·   16:9",
    ...
    sheet(rpngs, os.path.join(OUT, "reveal-order.png"), 420,
          ...
          "Video_5_Reveal_Builds   ·   %d frames   ·   duplicate sequential "
    ...
    SEC5 = {
        1:  ["A DIFFERENT EMPLOYER", "IS NOT THE ONLY WAY TO",
             "ACCESS DIFFERENT WORK.", "INTERNAL MOVE", "AUTOMATIC GROWTH."],
        2:  ["1", "WILL THE WORK CHANGE?", "2", "WILL YOUR JUDGMENT EXPAND?",
             "3", "WILL THE EVIDENCE TRAVEL?"],
        3:  ["1", "WILL THE WORK CHANGE?",
             "What will be different on an ordinary Monday?"],
        4:  ["NEW PROBLEMS", "NEW SYSTEMS", "NEW STAKEHOLDERS", "NEW CONTEXT"],
        5:  ["2", "WILL YOUR JUDGMENT", "EXPAND?",
             "What will you notice, weigh, recommend or own?"],
        6:  ["MORE TASKS", "Volume • Coordination • Absorption",
        ...
        8:  ["RESULT", "What changed?", "JUDGMENT",
             "What did you notice or decide?", "RANGE",
             "What new context can you now handle?"],
        9:  ["3 YES", "STRONG GROWTH CASE", "2 YES", "INVESTIGATE OR NEGOTIATE",
             "0-1 YES", "MOVEMENT, NOT MUCH FORMATION"],
        10: ["What problems will I own in the first six months?",
        ...
        12: ["ARE YOU GROWING—", "OR JUST BEING GIVEN", "MORE WORK?",
    ...
    rep["no_title_card"] = "Should I Make an Internal Move" not in " ".join(vis)
    rep["reveal_slides_specified"] = sorted(n for n in S.STEPS if S.STEPS[n] > 1)
    rep["reveal_spec_matches"] = rep["reveal_slides_specified"] == [1, 2, 4, 6, 8, 9, 10]
    ...
    rep["cta_name_on_slide_11"] = "CAREER DECISION EVIDENCE CHECK" in vis[10]
    rep["cta_url_slides"] = [i+1 for i,t in enumerate(vis)
                             if "temidayoafonja.com/career-decisions" in t]
    rep["no_competing_offer"] = not any(x in " ".join(vis).lower()
                                        for x in ("keep the proof", "field kit"))
    rev_vis = [flat("\n".join(sh.text_frame.text for sh in sl.shapes
                              if sh.has_text_frame and sh.text_frame.text.strip()))
               for sl in Presentation(rec_pptx).slides]
    rep["watch_next_in_last_frame"] = "MORE WORK?" in rev_vis[24]
    rep["main_and_reveal_wording_match"] = vis[11] == rev_vis[24]
