# -*- coding: utf-8 -*-
"""Literal comparison of the built v3.1 scripts against the canonical prompt."""
import sys, difflib
sys.path.insert(0,"/tmp/v5v31")

CANON="/tmp/v5v31/canonical.txt"
raw=open(CANON,encoding="utf-8").read().split("\n")
i=raw.index("BEGIN APPROVED VIDEO 5 v3.1 SCRIPT")
j=raw.index("END APPROVED VIDEO 5 v3.1 SCRIPT")
...
tel_raw=open("/tmp/v5v31/Video_5_HIT_FINAL/LONG_FORM/"
  "Video5TeleprompterScriptwithslidemarkers_HIT_v3.1.txt",encoding="utf-8").read()
...
# ---- reading TXT
rdg=[b.strip() for b in open("/tmp/v5v31/Video_5_HIT_FINAL/LONG_FORM/"
  "Video5ReadingScriptnomarkers_HIT_v3.1.txt",encoding="utf-8").read().split("\n\n") if b.strip()]
ok_rdg = rdg == spoken
...
teld=paras("/tmp/v5v31/Video_5_HIT_FINAL/LONG_FORM/"
  "Video5TeleprompterScriptwithslidemarkers_HIT_v3.1.docx")[4:]
teld_norm=["[SLIDE: %s]"%x[len("SLIDE  —  "):] if x.startswith("SLIDE  —  ") else x
           for x in teld]
print("teleprompter DOCX == canonical:", teld_norm==blocks)
rdgd=paras("/tmp/v5v31/Video_5_HIT_FINAL/LONG_FORM/"
  "Video5ReadingScriptnomarkers_HIT_v3.1.docx")[4:]
...
def short_copy(fname):
    """Re-extract one Short's spoken copy straight from the canonical file.
    v3.1 separates Shorts with bare `SHORT n` headings, not rule lines."""
    start=[k for k,l in enumerate(lines) if l.strip()=="FOUR STANDALONE SHORTS — v3.1"][0]
    k=[k for k,l in enumerate(lines) if l.strip()==fname and k>start][0]
    s0=[t for t,l in enumerate(lines) if l.strip()=="Spoken copy:" and t>k][0]
    e=len(lines)
    for t in range(s0+1,len(lines)):
        if _re.match(r"^SHORT \d$", lines[t].strip()) or lines[t].startswith("========"):
            e=t; break
    return [b.strip() for b in "\n".join(lines[s0+1:e]).split("\n\n") if b.strip()]
...
for fn,role,hook,copy in SHORTS:
    ...
    ok = canon_copy==copy
    allok &= ok
    built=paras("/tmp/v5v31/Video_5_HIT_FINAL/SHORTS/"+fn)
