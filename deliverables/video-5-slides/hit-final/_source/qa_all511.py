# -*- coding: utf-8 -*-
"""Run the v5.1 belonging/identity QA plus package, deck and rendering checks
across Videos 1-5."""
import sys, os, json, glob, zipfile, subprocess, re
sys.path.insert(0,"/tmp/v51"); sys.path.insert(0,"/tmp/da")
from qa511 import audit
...
CFG=json.load(open("/tmp/v51/qacfg.json"))
OUT={}
for n in range(1,6):
    B="/tmp/v51/v%d"%n
    cfg=CFG[str(n)]
    Rq,F,words,mk,spoken=audit(n,os.path.join(B,"canonical_v5.1.1.txt"),cfg)
    def chk(k,l,c,d=""):
    ...
    pub=glob.glob(ROOT+"/LONG_FORM/*Publishing*v5.1.1.docx")[0]
    edb=glob.glob(ROOT+"/LONG_FORM/*EDITOR_ONLY*v5.1.1.docx")[0]
    seb=glob.glob(SH+"/*Shorts_EDITOR_ONLY*v5.1.1.docx")[0]
    ...
    # Shorts
    chk("SH1","Four Shorts, all rewritten for v5.1.1", len(shorts)==4, len(shorts))
    ...
    chk("P4","All notes parts carry v5.1.1 narration",
...
json.dump(OUT,open("/tmp/v51/QA_REPORT_v511.json","w"),indent=1)
