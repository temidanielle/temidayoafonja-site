"""Build Video 2 version 1.1 and run its QA.

One copy change on slide 2. Version 1.0 files are never touched.
...
import slides_v1_1 as S
from PIL import Image, ImageDraw, ImageFont, ImageChops

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
OUT = os.path.join(ROOT, "out")
V11 = os.path.join(OUT, "v1.1")
PNG = os.path.join(V11, "png")
REV = os.path.join(V11, "reveals")
GUIDES = os.path.join(V11, "guides")
...
def main():
    for d in (OUT, V11, PNG, REV, GUIDES, RENDER):
    ...
    for i, (n, s) in enumerate(RECORDING_PLAN):
        ...
        rlabels.append(t)
    sheet(rpngs, os.path.join(V11, "recording-deck-order-v1.1.png"), 420,
          "RECORDING DECK, ADVANCE ORDER",
          "Video-3-Reveal-Builds_v1.1   ·   %d frames   ·   sequential duplicate "
          "slides, no animations" % len(rpngs), labels=rlabels)
    sheet(pngs, os.path.join(V11, "phone-thumbnail-check-v1.1.png"), 320,
    ...
    checks = ["Preserve the evidence", "Name what the work built",
              "Test the next move"]
    def only(i, has, hasnt):
        return all(h in vis[i - 1] for h in has) and not any(h in vis[i - 1]
                                                             for h in hasnt)
    rep["section_01_alone"] = only(3, ["THREE CHECKS BEFORE YOU RESIGN", "01",
                                       checks[0]], [checks[1], checks[2]])
    rep["section_02_alone"] = only(5, ["THREE CHECKS BEFORE YOU RESIGN", "02",
                                       checks[1]], [checks[0], checks[2]])
    rep["section_03_alone"] = only(7, ["THREE CHECKS BEFORE YOU RESIGN", "03",
                                       checks[2]], [checks[0], checks[1]])
    rep["slides_with_all_three"] = [i + 1 for i, t in enumerate(vis)
                                    if all(x in t for x in checks)]
    rep["recap_only"] = rep["slides_with_all_three"] == [9]
    rep["section_slides_have_no_builds"] = all(S.STEPS[n] == 1 for n in (3, 5, 7, 9))
    rep["cta_route"] = [i + 1 for i, t in enumerate(vis)
                        if "temidayoafonja.com/career-decisions" in t]
    rep["fieldkit_absent"] = not any("fieldkit" in t.lower() for t in vis + nts)
    rep["safety_boundary_on_slide"] = (
        "If your health or safety is at risk, or you are facing harassment or "
        "discrimination, this is not a reason to wait." in vis[1])
    rep["safety_boundary_in_notes"] = sum(
        1 for t in nts if "health or safety" in t or "harmful situation" in t)
    rep["not_take_boundary"] = all(x in vis[3] for x in
    ...
    rep["slide4_notes_entitlement"] = ("if you are not entitled to keep it, do not "
                                       "take it." in nts[3])
    rep["slide4_notes_material_line"] = ("Preserving your record does not mean taking "
                                         "their material." in nts[3])
    rep["legal_heuristic_gone"] = not any("legal team" in t for t in vis + nts)
    rep["legible_line"] = ("The point is not to make the decision slow. The point "
                           "is to make it legible." in vis[9])
    rep["watch_next"] = "How to Change Jobs Without Starting Your Career Over" in vis[12]
    banned = ["\u2014", "\u2013", "r\u00e9sum\u00e9"]
    ...
    with open(os.path.join(V11, "qa-raw-v1.1.json"), "w") as f:
    ...
    print("safety boundary on slide 2:", rep["safety_boundary_on_slide"],
    ...
    print("slide 4 notes entitlement:", rep["slide4_notes_entitlement"],
          "| material line kept:", rep["slide4_notes_material_line"],
          "| legal heuristic gone:", rep["legal_heuristic_gone"])
    print("CTA route on slides:", rep["cta_route"],
          "| Field Kit absent:", rep["fieldkit_absent"])
