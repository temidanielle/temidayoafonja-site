# -*- coding: utf-8 -*-
"""Document builders for the NEW V10 and V11 packages.
...
EYEBROW = "capability formation | new public"


def label(n):
    return "NEW PUBLIC V%d" % n


def header(d, n, kind, stamp):
    title_block(d, EYEBROW, S.title(n), kind)
    kv(d, "New public number", "V%d" % n)
    kv(d, "Former roadmap number", "None. This is a new "
                                   "concept.")
...
def recording_master(n, path, stamp):
    d = base_doc()
    header(d, n, "FINAL recording master", stamp)
    callout(d, "This is the approved FINAL script, word for word. The "
...
def sound_map(n, path):
    L = head("NEW PUBLIC V%d  |  AUDIO AND SOUND CUE MAP" % n)
    L += [S.title(n), "",
...
BROLL = {
 10: [("Hands and a notebook, writing a short list",
       "Under the READ section, where the four things are being named.",
       "Full screen, brief. No text legible, and nothing that implies a "
       "specific employer."),
      ("A quiet desk in an unfamiliar room, nobody in frame",
       "Under WHY THIS FEELS STRANGE, on the line about not knowing which "
       "meeting matters.",
       "Full screen, short. No office stock energy, no people, no "
       "signage."),
      ("A doorway or corridor before a meeting",
       "Under THE CONVERSATION I WOULD HAVE, if the cut needs air before "
       "the manager questions.",
       "Full screen, brief. Never behind the questions card.")],
 11: [("Two sheets of paper side by side on a desk",
       "Under the EXPECTED and ACTUAL sections, where the two columns are "
       "being described.",
       "Full screen, brief. No text legible. It suggests the exercise "
       "without pretending to be the viewer's own."),
      ("A window or a walk, mid-afternoon, nobody in frame",
       "Under THE BOUNDARY, where the script says the framework does not "
       "tell you whether to leave.",
       "Full screen, short. Nothing that reads as resignation or as "
       "defeat."),
      ("Hands closing a notebook",
       "Under the CLOSE, if the cut needs a breath before Watch Next.",
       "Full screen, brief. Never behind the role-drift read card.")],
}

BROLL_LIMITS = {
 10: "No office stock energy, no people in frame, no signage, no "
     "identifiable workplace. Nothing that reenacts a manager "
     "conversation, and nothing that implies a specific employer.",
 11: "Nothing that frames the employer or the manager as a villain: no "
     "closed door read as a rejection, no packed box, no resignation "
     "imagery. Nothing that implies a specific employer.",
}


def broll(n, path):
    L = head("NEW PUBLIC V%d  |  B-ROLL NOTES" % n)
    L += [S.title(n), "",
