VIDEO 8 - FIRST-PASS PRODUCTION PACKAGE - QA README
...
1.4  WORD-COUNT METHOD DOCUMENTED AND APPLIED CONSISTENTLY
  ...
  Word count after the corrections: 1586, down from 1,607. Spoken paragraphs
  remain 38, as required.

1.5  PAGINATION REPAIRED STRUCTURALLY - AND A LIMITATION TO STATE PLAINLY

  Package DOCX:
    - w:cantSplit on every table row - all 112 of them - so no row can
...
Verified by search across the finished script:
  "18 years" and "eighteen years" absent           true
  "seamless" absent                                true
  "transfer automatically" absent                  true
  promotion-in-every-industry claim absent         true
  30% and $2M+ absent                              true / true
  CISM stated only within the ceiling              true
...
FLUSH EDGES, REPORTED SEPARATELY FROM TRUE OVERHANG
  ...
  main slides with a flush edge          2, 8
  reveal frames with a flush edge        4, 16
...
are generated from it, so the three cannot drift apart.
  ...
  package spoken paragraphs              38
  teleprompter spoken paragraphs         38
  clean TXT paragraphs                   38
  clean TXT == package                   true
  teleprompter == package                true
  clean TXT == teleprompter              true
  spoken word count                      1586   (target 1,450-1,700)
  ...
  locked opening distinction present     true
  translation sentence present           true
  three columns named in the script      true
  ...
  confidentiality boundary spoken        true
  fabrication explicitly ruled out       true
