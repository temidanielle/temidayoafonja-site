...
import p3_blocks as B, p3_shorts as S, p3_production as PR, p3_descriptions as D, p3_packaging as PK
import p3_checks_script as CH
...
def q20(c):
    """The delivery manifest names V14's provenance file V14_FINAL_SOURCE_PROVENANCE.docx,
    with no _WORKING suffix, so the filename rule applies to the five packaging-dependent
    files and the in-document label applies to all six. An earlier draft required the
    suffix everywhere and flagged a file that is named exactly as instructed."""
    SUFFIXED = ("master", "blocks", "prod", "shorts", "desc")
    ALL = SUFFIXED + ("prov",)
    bad = []
    for kind in SUFFIXED:
        if "_WORKING" not in os.path.basename(c.p(14, kind)):
            bad.append("filename not marked: " + os.path.basename(c.p(14, kind)))
    for kind in ALL:
        path = c.p(14, kind)
    ...
    for n in (12, 13):
        for kind in ALL:
            if any("PACKAGING PENDING" in t for t in paras(c.p(n, kind))):
                bad.append("V%d must not be marked working: %s" % (n, kind))
    probe = not any("PACKAGING PENDING TEMIDAYO APPROVAL" in t
                    for t in paras(c.p(12, "master")))
    return PERFORMED, (not bad and probe), bad or ("Five packaging-dependent V14 files carry "
            "_WORKING in the filename, all six carry the label on page one, and no V12 or V13 "
            "file carries it.")
...
NOT_DONE = [
 ...
 ("V14 packaging", "Not chosen. Three candidates are assessed against the reconciled script and "
                   "one is recommended for approval. Every V14 file stays marked working."),
