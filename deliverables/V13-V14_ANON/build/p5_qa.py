...
import p5_blocks as B, p5_shorts as S, p5_production as PR, p5_descriptions as D, p5_packaging as PK
import p5_provenance as PV
import p5_docs as DP
import p5_checks_script as CH
...
EMPLOYERS = ["humana", "wells fargo", "mass general", "brigham", "jpmorgan",
             "j.p. morgan", "jp morgan", "chase", "center for disaster medicine"]

@item(27, "No employer name survives in any public V13 or V14 asset")
def q27(c):
    bad = []
    for n in (13, 14):
        for kind in ("master", "blocks", "prod", "shorts", "desc"):
            body = " ".join(paras(c.p(n, kind))).lower()
            bad += ["V%d %s: %s" % (n, kind, e) for e in EMPLOYERS if e in body]
    probe = "humana" in "the first, from humana, says"
    return PERFORMED, (not bad and probe), bad or (
        "Nine employer strings searched across the recording master, thought "
        "blocks, production package, Shorts and description for both videos.")

@item(28, "Private provenance still names the employers")
def q28(c):
    keep = ["Humana", "Wells Fargo", "Mass General Brigham",
            "JPMorganChase"]
    miss = [k for k in keep
            if not any(k in v for p in PV.POSTINGS.values() for v in p.values())]
    return PERFORMED, not miss, miss or (
        "All four employers, their exact titles, URLs, collection dates, "
        "posting dates, capture routes and hard-against-preferred coding are "
        "unchanged in the provenance module.")

@item(29, "The neutral labels do not flatten the postings together")
def q29(c):
    b13 = " ".join(_spoken_from_master(c.p(13, "master"))).lower()
    ok = ("health insurer" in b13 and "a bank" in b13
          and "hospital system" in b13
          and b13.count("healthcare posting") == 0)
    return PERFORMED, ok, (
        "V13 keeps three distinct sectors: a health insurer, a bank and a "
        "hospital system. Calling the insurer and the hospital both a "
        "healthcare posting would have erased the contrast the episode "
        "teaches.")

@item(30, "No claim was generalized to an industry by the anonymization")
def q30(c):
    pats = [r"health insurers (think|want|require)", r"banks (think|want|require)",
            r"hospital systems (think|want|require)", r"the industry requires",
            r"insurers all", r"every bank"]
    bad = []
    for n in (13, 14):
        body = " ".join(_spoken_from_master(c.p(n, "master"))).lower()
        bad += ["V%d: %s" % (n, p) for p in pats if re.search(p, body)]
    return PERFORMED, not bad, bad or (
        "The labels stay attached to one posting each. Six generalization "
        "patterns searched.")

@item(31, "Nothing but the anonymization changed in V13 and V14")
def q31(c):
    """Word counts move only by the length of the substituted labels."""
    deltas = {13: DP.words(13) - 1457, 14: DP.words(14) - 1449}
    ok = all(0 <= d <= 12 for d in deltas.values())
    return PERFORMED, ok, (
        "V13 1,457 to %d spoken words, V14 1,449 to %d. Both deltas are "
        "accounted for entirely by the replacement labels being longer than "
        "the employer names." % (DP.words(13), DP.words(14)))
