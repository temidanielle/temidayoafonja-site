...
MODS = {12: "p5_script12", 13: "p5_script13", 14: "p5_script14"}
...
# Superseded by the lock: V13 now names V14's approved title on camera instead of
# describing the episode. The replacement lives in the locked-packaging block below.
...
# --- 18. locked packaging is what the scripts route to
LOCKED_TITLES = {
...
for n in MODS:
    check("V%d carries its locked title" % n,
          _il.import_module(MODS[n]).TITLE == LOCKED_TITLES[n],
          _il.import_module(MODS[n]).TITLE)
check("V12 watch-next names V13's locked title",
      LOCKED_TITLES[13] in text(12))
check("V13 watch-next names V14's locked title",
      LOCKED_TITLES[14] in text(13))
check("V14 watch-next names V12's locked title",
      LOCKED_TITLES[12] in text(14))
check("V13 no longer routes to V14 by description alone",
      "two real postings side by side" not in text(13).lower())

# --- 19. no working-packaging language survives anywhere in the scripts
for n in MODS:
    t = text(n).lower()
    hits = [p for p in ("proves / suggests", "packaging pending", "[working]", "working title")
            if p in t]
    check("V%d carries no working-packaging language" % n, not hits, hits)
check("V14 status field is cleared", _il.import_module(MODS[14]).STATUS is None)
