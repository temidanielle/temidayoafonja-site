...
spellings my v2.2.1 sweep had missed. She was right: that sweep only covered the
source modules I had edited, not the visible slide text (generated by a separate
build script) or the boilerplate in build.py that produces the brief and README.

A full case-insensitive audit across every Video 8 surface - visible slide text,
reveal text, speaker/editor notes, long-form scripts, Shorts, description,
publishing package, editor brief, README and summary - found exactly 14
occurrences in 7 forms, all ordinary prose, no proper nouns or quotations:

| Old | New | Count | Where |
|---|---|---|---|
| `recognise` | `recognize` | 2 | **VISIBLE SLIDE** - main slide 7 and reveal frame 14, "Which patterns do you already recognise early?" |
| `programme` | `program` | 3 | speaker notes: main slide 8, reveal frames 15 and 16 |
| `authorised` | `authorized` | 3 | README, summary |
| `Authorised` | `Authorized` | 1 | editor brief |
| `AUTHORISED` | `AUTHORIZED` | 2 | editor brief, summary |
| `authorisation` | `authorization` | 1 | README |
| `colours` | `colors` | 2 | README, editor brief |

**Editorial content is untouched.** The long-form reading script is byte-identical
to before the sweep and all four Shorts are content-identical. Word count stays
1,890 and runtime 13:02. No structure, teaching, title, thumbnail, CTA or
packaging strategy changed.

The visible slide fix was applied with the same discipline as the v2.2.1
corrections: a single `<a:t>` run edit per file, with position, size, font,
weight, colour and `<a:br/>` count asserted identical before and after. Part
diff: `slide7.xml` on the main deck and `slide14.xml` on the reveal deck, one
part each. Slide 7 was re-rendered and visually checked.

#### Two things worth recording

`qa.py` and `verify_v22.py` hold British spellings deliberately, as detection
targets in their US-English check lists. My first sweep converted those too,
which would have made the checks tautological - always passing. Both files were
restored and only their comment/label prose was converted. **A spelling sweep
must never be run blindly over its own test fixtures.**

QA check 14.3 was widened from 4 literal words to 18 case-insensitive British
stems covering decks, scripts, Shorts, briefs, README and notes, so this class of
miss is caught automatically rather than by eye. Check 17 now authorizes slides
2, 4 and 7 (reveal 4, 9, 14).

48 of 48 checks pass. An independent re-audit across all surfaces returns clean.

#### Final hashes

| Asset | SHA-256 |
|---|---|
| Package ZIP | `9b100f80354fefd4720c4878c823dba33f11cc921a75cf722c620048f4de25e8` |
| Description DOCX | `4d266a53bd3dc60f184f6b5bb869b5dd0cafb41f766a8b3a1ec73a8588cb3b9b` |
| Summary DOCX | `787c5dc164bdc3594b65693cdd0a6a60789c31efbb625f6cf4690b69741bbb51` |
| Main deck | `3eef0fc02597ab53e0145c7ead848e7cedd7e177c706b7534645b40abe051832` |
| Reveal deck | `9b0b2953090f77e87e65844e337b563c99bd2573fcaff1d9a4e86453c6500f1b` |

The eight loose recording scripts were refreshed from the rebuilt package.
