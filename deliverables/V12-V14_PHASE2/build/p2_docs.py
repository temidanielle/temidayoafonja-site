...
import p2_shorts as S
import p2_production as PR
import p2_descriptions as D
import p2_provenance as PV
import p2_packaging as PK
import p2_checks_script as CH

EYEBROW = "capability formation | phase 2 production"
STAMP = "Monday, September 21, 2026"

META = {
12: dict(number="V12", title="Turn One Accomplishment Into Proof in 10 Minutes",
...
14: dict(number="V14", title="What Your Experience Proves, and What It Only Suggests",
...
def _header(d, n, kind):
    ...
    if m["status"]:
        callout(d, m["status"] + ". The title, thumbnail and every line that names them are a "
                                 "working assumption only. See V14_PACKAGING_OPTIONS.")
...
def production_package(path, n):
    ...
    para(d, "%s is a full-screen card and is the final frame of the video. Copy: %s"
          % (wn[0], wn[2][0]))
    if n == 13:
        callout(d, "V13's Watch Next is spoken as a description rather than a title, because V14's "
                   "packaging is not approved yet. The card carries the title and can be re-set "
                   "without a re-record.")
