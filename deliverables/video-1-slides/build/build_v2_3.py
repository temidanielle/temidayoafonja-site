"""Build version 2.3 and run the URL-change QA.

Version 2.3 changes one line: the slide 9 Field Kit destination becomes
temidayoafonja.com/fieldkit. Nothing else changes.

Outputs (version 2.2 files are never touched):
  ...
  out/v2.3/...  supporting renders, sheets, guides and raw QA

    python3 build/build_v2_3.py
"""
...
import slides_v2_3 as S
from PIL import Image, ImageDraw

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
OUT = os.path.join(ROOT, "out")
V = os.path.join(OUT, "v2.3")
PNG = os.path.join(V, "png")
REV = os.path.join(V, "reveals")
GUIDES = os.path.join(V, "guides")
DECK = "Video-1-How-I-Changed-Jobs-Without-Starting-My-Career-Over_v2.3"
RECDECK = "Video-1-Reveal-Builds_v2.3"
N = 10
...
def recording_sheet(paths, plan, out_path):
    ...
    d.text((pad, 56), "RECORDING DECK, ADVANCE ORDER", font=font(34, True),
           fill=(15, 35, 70))
    d.text((pad, 104), "Video-1-Reveal-Builds_v2.3   ·   %d frames   ·   "
...
def main():
    ...
    html = render_html(cvs, os.path.join(RENDER, "deck_v2_3.html"),
                       "How I Changed Jobs Without Starting My Career Over (v2.3)")
    ...
    rhtml = render_html(rcvs, os.path.join(RENDER, "recording_v2_3.html"),
                        "Recording deck v2.3")
    ...
    _sheet(pngs, os.path.join(V, "contact-sheet-v2.3.png"), 560,
           "HOW I CHANGED JOBS WITHOUT STARTING MY CAREER OVER",
           "Version 2.3   ·   10 slides   ·   1920 x 1080   ·   16:9")
    recording_sheet(rpngs, RECORDING_PLAN, os.path.join(V, "recording-deck-order-v2.3.png"))
    _guides(pngs, GUIDES)
    ...
    rep["slide2_stages"] = [t for t in flat(vis[1]).split("  ") if t]
    rep["slide2_four_stages"] = all(
        x in flat(vis[1]) for x in ("ACCOUNTING & AUDIT", "CYBERSECURITY",
                                    "PEOPLE STRATEGY", "CAPABILITY FORMATION"))
    rep["slide2_old_stages_demoted"] = (
        "LIFE SCIENCES" not in flat(vis[1]) and "CONSULTING" not in flat(vis[1])
        and "Along the way: consulting, life sciences, technology" in flat(vis[1]))
    rep["slide2_bridge_kept"] = ("I carried that way of seeing into cybersecurity and "
                                 "later into people strategy." in flat(vis[1]))
    rep["slide3_renamed"] = ("THREE THINGS I LEARNED TO DO" in flat(vis[2])
                             and "THE THREE MOVES" not in flat(vis[2]))
    rep["slide3_support_line"] = ("These helped me carry my experience forward."
                                  in flat(vis[2]))
    rep["slide5_label"] = ("What another employer can understand" in flat(vis[4])
                           and "Portable description" not in flat(vis[4]))
    rep["slide9_url"] = ("temidayoafonja.com/fieldkit" in flat(vis[8])
                         and "temidayoafonja.com/book" not in flat(vis[8]))
    rep["url_on_slides"] = [i + 1 for i, t in enumerate(vis)
                            if "temidayoafonja.com/fieldkit" in flat(t)]
    rep["slide6_unchanged_text"] = flat(vis[5])
    rep["slide6_notes"] = flat(nts[5])
    rep["move_kickers_kept"] = all(k in flat(vis[i]) for i, k in
                                   ((3, "MOVE ONE"), (4, "MOVE TWO"), (6, "MOVE THREE")))
    ...
    with open(os.path.join(V, "qa-raw-v2.3.json"), "w") as f:
    ...
    print("slide 2 four stages:   ", rep["slide2_four_stages"],
          "| settings demoted:", rep["slide2_old_stages_demoted"],
          "| bridge copy kept:", rep["slide2_bridge_kept"])
    print("slide 3 renamed:       ", rep["slide3_renamed"],
          "| support line:", rep["slide3_support_line"])
    print("slide 5 label:         ", rep["slide5_label"],
          "| MOVE ONE/TWO/THREE kept:", rep["move_kickers_kept"])
    print("slide 9 url = /fieldkit:", rep["slide9_url"],
          "| url appears on slides:", rep["url_on_slides"])
