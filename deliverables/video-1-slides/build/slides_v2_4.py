"""
Video 1 deck, version 2.4. Thirteen slides.
...
Version 2.1 made two corrections only: the slide 2 capability bridge now
...
Version 2.2 is a targeted refinement of three items, nothing else:
  - slide 2 timeline reduced to the four substantive stages, with the other
    settings demoted to one muted context line
  - slide 3 kicker becomes "Three things I learned to do", so the label
    cannot be read as three job changes
  - slide 5 right-hand label becomes "What another employer can understand",
    so the teaching point needs no Capability Formation vocabulary

Version 2.3 changed one line: the slide 9 destination becomes
temidayoafonja.com/fieldkit, the route netlify.toml already defines for the
Field Kit, approved after the version 2.2 URL check.

Version 2.4 corrects the structure of the three ideas. They were reveal
states of one overview slide, which meant idea one was still on screen while
idea two arrived. They are now three standalone section-break slides at three
separate moments in the video, each carrying only its own idea, and the old
three-card composition becomes a recap that runs once, after Move Three, when
the viewer has learned all three. Every other slide is carried over unchanged.
"""
import os
from deck import *
...
# The three section breaks never build; they are single, separate moments.
STEPS = {1: 1, 2: 1, 3: 1, 4: 3, 5: 1, 6: 2, 7: 2, 8: 1, 9: 4, 10: 1, 11: 3,
         12: 1, 13: 1}

TITLES = {
    1: "Title",
    2: "Career path and capability bridge",
    3: "Section break 01, look underneath the title",
    4: "Move one, look underneath the title",
    5: "Section break 02, explain what the work changed",
    6: "Move two, explain what the work changed",
    7: "Move two evidence, 47 to 75",
    8: "Section break 03, keep evidence before you need it",
    9: "Move three, keep evidence",
    10: "Recap, all three together",
    11: "Three-question exercise",
    12: "Field Kit invitation",
    13: "Next-video bridge",
}

SLUGS = {
    1: "title", 2: "career-path", 3: "section-01-look-underneath",
    4: "move-1-look-underneath", 5: "section-02-explain-what-changed",
    6: "move-2-translation", 7: "move-2-evidence-47-to-75",
    8: "section-03-keep-evidence", 9: "move-3-keep-evidence",
    10: "recap-three-things", 11: "three-question-exercise",
    12: "fieldkit-invitation", 13: "next-video-bridge",
}
...
def slide_02(sl, step=1):
    ...
    spine_y, x0, x1 = 862, 320, 1610
    hairline(sl, x0, spine_y, x1 - x0, color=BLUE, h=3)
    stages = ["Accounting & audit", "Cybersecurity", "People strategy",
    ...
    for i, label in enumerate(stages):
        ...
        block(sl, cx - 210, spine_y + 42, 420, [
            (label.upper(), dict(size=26, bold=True, tracking=2, align="c",
                                 color=NAVY if last else NAVY_DIM, spacing=1.2)),
        ])
    # settings worked in along the way, deliberately subordinate to the stages
    block(sl, x0 - 210, spine_y + 102, (x1 - x0) + 420, [
        ("Along the way: consulting, life sciences, technology",
         dict(size=22, font=BODY, color=NAVY_DIM, align="c")),
    ])
...
def _section_intro(sl, number, statement, support, notes_text):
    """A section break: one number, one statement, air. No cards, no reveal."""
    bg(sl, CREAM)
    eyebrow(sl, 120, 452, "Three things I learned to do", w=1400)
...
def section_01(sl, step=1):
    _section_intro(sl, 1, "Look underneath\nthe title",
                   "What has the work actually trained you to do?", """Timing: 1:35-1:43

A section break, four to seven seconds, while you name the first idea.

Do not teach it here. The Move One slide follows.

Only this idea is on screen. The second and third come later, each in its
own moment.""")


def section_02(sl, step=1):
    _section_intro(sl, 2, "Explain what\nthe work changed",
                   "What changed because you were there?", """Timing: 3:05-3:13

A section break, four to seven seconds, while you name the second idea.

Do not restate the first idea. Move Two follows.""")


def section_03(sl, step=1):
    _section_intro(sl, 3, "Keep evidence\nbefore you need it",
                   "Capture the context, your role and what changed.", """Timing: 4:50-4:58

A section break, four to seven seconds, while you name the third idea.

Do not restate the first two. Move Three follows.""")


# ---------------------------------------------------------- recap slide
def slide_recap(sl, step=1):
    bg(sl, CREAM)
    kicker_head(sl, "Three things I learned to do",
                "What helped me carry\nmy experience forward.", head_size=54)
    cards = [
        ("01", "Look underneath\nthe title"),
        ("02", "Explain what\nthe work changed"),
        ("03", "Keep evidence\nbefore you need it"),
    ...
    notes(sl, """Timing: 6:25-6:40

The first and only moment all three appear together.

Name them once, in order, then move to the viewer exercise. Do not teach
them again.""")
...
def slide_09(sl, step=1):
    ...
    block(sl, 120, 846, 720, [
        ("temidayoafonja.com/fieldkit", dict(size=40, bold=True, color=CREAM, align="c")),
    ], anchor="m", h=96)
...
BUILDERS = {1: slide_01,       2: slide_02,     3: section_01,
            4: slide_04,       5: section_02,   6: slide_05,
            7: slide_06,       8: section_03,   9: slide_07,
            10: slide_recap,  11: slide_08,    12: slide_09,
            13: slide_10}
