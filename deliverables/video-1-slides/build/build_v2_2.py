"""Build version 2.2 and run the refinement QA.

Version 2.2 refines three items on the version 2.1 deck: the slide 2 career
timeline, the slide 3 kicker and headline, and the slide 5 right-hand label.
Nothing else changes.

Outputs (version 2.1 files are never touched):
  ...
  out/v2.2/...  supporting renders, sheets, guides and raw QA

    python3 build/build_v2_2.py
"""
...
import slides_v2_2 as S
from PIL import Image, ImageDraw

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
OUT = os.path.join(ROOT, "out")
V = os.path.join(OUT, "v2.2")
PNG = os.path.join(V, "png")
REV = os.path.join(V, "reveals")
GUIDES = os.path.join(V, "guides")
DECK = "Video-1-How-I-Changed-Jobs-Without-Starting-My-Career-Over_v2.2"
RECDECK = "Video-1-Reveal-Builds_v2.2"
N = 10
...
def recording_sheet(paths, plan, out_path):
    ...
    d.text((pad, 56), "RECORDING DECK, ADVANCE ORDER", font=font(34, True),
           fill=(15, 35, 70))
    d.text((pad, 104), "Video-1-Reveal-Builds_v2.2   ·   %d frames   ·   "
...
def main():
    ...
    html = render_html(cvs, os.path.join(RENDER, "deck_v2_2.html"),
                       "How I Changed Jobs Without Starting My Career Over (v2.2)")
    ...
    rhtml = render_html(rcvs, os.path.join(RENDER, "recording_v2_2.html"),
                        "Recording deck v2.2")
    ...
    _sheet(pngs, os.path.join(V, "contact-sheet-v2.2.png"), 560,
           "HOW I CHANGED JOBS WITHOUT STARTING MY CAREER OVER",
           "Version 2.2   ·   10 slides   ·   1920 x 1080   ·   16:9")
    recording_sheet(rpngs, RECORDING_PLAN, os.path.join(V, "recording-deck-order-v2.2.png"))
    _guides(pngs, GUIDES)
    ...
    with open(os.path.join(V, "qa-raw-v2.2.json"), "w") as f:
