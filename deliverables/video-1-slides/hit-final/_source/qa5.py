# -*- coding: utf-8 -*-
"""v5.0 belonging / identity / simple-language QA for a single video script."""
import sys, re
...
def audit(n, path, cfg):
    nb=new_blocks(path,"BEGIN APPROVED VIDEO %d v5.0 SCRIPT"%n,
                       "END APPROVED VIDEO %d v5.0 SCRIPT"%n)
    ...
    long_words=[x for x in re.findall(r"[A-Za-z]{13,}",text)
                if x.lower() not in ("understanding","understandable","professionals",
                                     ...
                                     "contribution","specifically","automatically")]
