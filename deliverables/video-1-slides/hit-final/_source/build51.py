# -*- coding: utf-8 -*-
"""Generic v5.1 package builder. Each video supplies a config module; the
...
LINES=new_blocks(os.path.join(BASE,"canonical_v5.1.txt"),
  "BEGIN APPROVED VIDEO %d v5.1 SCRIPT"%N,"END APPROVED VIDEO %d v5.1 SCRIPT"%N)
...
TEL="Video%dTeleprompterScriptwithslidemarkers_HIT_v5.1"%N
RDG="Video%dReadingScriptnomarkers_HIT_v5.1"%N
EDB="Video_%d_EDITOR_ONLY_HIT_Brief_v5.1.docx"%N
PUB="Video_%d_Publishing_Package_HIT_v5.1.docx"%N
SEB="Video_%d_Shorts_EDITOR_ONLY_HIT_Brief_v5.1.docx"%N
scripts(N,C.TITLE,LINES,SPOKEN,LF,TEL,RDG)

# ---------------------------------------------------------- editor brief
d=newdoc()
P(d,"EDITOR ONLY",size=22,bold=True,color=RED,after=2)
P(d,"VIDEO %d  ·  v5.1 BELONGING + IDENTITY"%N,size=12,bold=True,color=GOLD,
  after=2,caps=True)
...
pairlist(d,["Main deck: %d notes parts rewritten for the v5.1 narration."%C.SLIDES,
...
for kind in ("pub","desc"):
    d=newdoc()
    head(d,N,C.TITLE,"Video %d  ·  %s  ·  v5.1"%(N,"Publishing package"
...
for (fn,role,hook,copy),label in zip(SHORTS,["SHORT 1","SHORT 2","SHORT 3","SHORT 4"]):
    d=newdoc(True)
    P(d,"VIDEO %d SHORT  ·  v5.1"%N,size=10,bold=True,color=GOLD,after=4,caps=True)
...
d=newdoc()
P(d,"EDITOR ONLY",size=22,bold=True,color=RED,after=2)
P(d,"VIDEO %d — FOUR STANDALONE SHORTS  ·  v5.1"%N,size=18,bold=True,
...
keep(P(d,"Separately recorded 9:16 Shorts. NOT excerpts cut from the long-form "
       "video. All four were rewritten for v5.1.",bold=True,after=10))
...
R=["VIDEO %d — v5.1 BELONGING + IDENTITY FINAL RECORDING PACKAGE"%N,""]
...
z=package(ROOT,MANIFEST,ZIP,"Video_%d_HIT_FINAL"%N,
  ["# VIDEO %d - v5.1 BELONGING + IDENTITY FINAL RECORDING PACKAGE"%N,
