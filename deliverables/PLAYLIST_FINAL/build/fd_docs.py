...
def script_doc(v, path):
    ...
    for line in v["roadmap"]:
        _p(d, line, 12, False, None, 0, 8)
    _p(d, "These lines match the chapter titles word for word, apart from "
          "the \u201cStep 1\u201d style numbering the chapter list adds. If "
...
def riverside_doc(v, path):
    ...
    line("%s  \u00b7  publishes %s  \u00b7  recorded with the other two in "
         "one session this week  \u00b7  one page, keep it beside the camera"
    ...
    hh("photo stills \u00b7 do this before you strike the lights")
    line("After the last take, still in the same light and framing, capture 3 "
         "to 5 stills at full camera resolution. These are the source for the "
         "Canva thumbnails, so shoot them properly rather than pulling a "
         "frame from the video later.", 8.5)
    line("Direction for this video:  " + D.THUMB_DIRECTION[v["n"]],
         8.5, True, NAVY_RGB, before=3)
    line("\u00b7  Chest height, eyes on the top third, same background as "
         "the recording.", 8.5, ind=0.1)
    line("\u00b7  Leave clear space on one side of the frame for the "
         "thumbnail text.", 8.5, ind=0.1)
    line("\u00b7  Shoot all three videos\u2019 stills in the same session so "
         "the set matches.", 8.5, ind=0.1)

    hh("session")
    line("All three videos in one session this week. Video 1, short break, "
         "Video 2, short break, Video 3, then the stills. About 2.5 to 3 "
...
def upload_doc(v, path):
    ...
    if v["n"] == 1:
        _p(d, "Create the playlist before %s so Video 1 launches inside it."
           % v["publish"].replace(", 2026", ""), 9.5, True, GOLD_RGB, 4, 6)
    ...
    h(d, "thumbnail text")
    _p(d, "No thumbnail image ships with this kit. Build it in Canva from the "
          "stills the Riverside sheet asks for. These are the exact words and "
          "the photo direction, and nothing else.", 10.5, after=6)
    _p(d, "  ".join(v["thumb_text"]), 15, True, NAVY_RGB, 2, 6)
    kv(d, "Photo direction", D.THUMB_DIRECTION[v["n"]])
    kv(d, "Set", "Same background, framing and color treatment as the other "
                 "two, so the playlist reads as one series.")
    kv(d, "Check", "Readable at 160 pixels wide.")
    ...
    for c in ["Captions: corrected SRT uploaded, not auto-generated.",
              ...
              "Thumbnail built in Canva and checked at 160 pixels wide.",
...
def shorts_doc(v, path):
    ...
    for c in clips:
        ...
        _p(d, "Key line to show on screen", 9.5, True, GOLD_RGB, 6, 2)
        _p(d, c["key_line"], 12, True, NAVY_RGB, 0, 4, indent=0.2)
        _p(d, "Hold this one full width while she says it. It is the line the "
              "clip exists for.", 9, False, GREY_RGB, 0, 4, italic=True)
