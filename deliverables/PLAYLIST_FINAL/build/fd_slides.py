...
No production label is drawn on a viewer-facing slide. "Title card", "End card",
"Roadmap" and the like are names for the editor, not for the audience, so they
live in the `label` field for the cut list and never reach the screen. A label
a viewer benefits from, such as STEP 1, is drawn. SHOW_LABEL decides which.
...
def shows_label(label):
    """True when the eyebrow is worth putting in front of a viewer."""
    import re
    return bool(re.match(r"^(step \d+)$", label.strip(), re.I))
...
1: [
 dict(no=1, name="V1_01_TITLE_CARD", kind="title", at="0:00",
      ...
      label="Title card",
      head="How to Get a New Job",          # RELEASE EDIT: year removed
 ...
 dict(no=2, name="V1_02_ROADMAP", kind="roadmap", at="0:30",
      ...
      head="5 steps before your resume",
 ...
 dict(no=4, name="V1_04_STEP_2", kind="list", at="3:30", when="3:30",
      label="Step 2",
      head="Build your evidence before your resume",
...
1: [
 dict(no=1, name="V1_SHORT_1_MEMORY_IS_NOT_A_RECORD",
      key_line="Your memory is not a record.",
      source="Video 1, Step 2: Build your evidence before your resume",
 ...
 dict(no=2, name="V1_SHORT_2_LOWER_TITLE",
      key_line="A smaller title with bigger practice can be the stronger move.",
...
2: [
 dict(no=1, name="V2_SHORT_1_THE_SCRIPT",
      key_line="The part I would still need to learn is this, and here is how I would learn it in the first ninety days.",
...
3: [
 dict(no=1, name="V3_SHORT_1_A_FEW_PEOPLE",
      key_line="You need a few people who can describe what you do well.",
