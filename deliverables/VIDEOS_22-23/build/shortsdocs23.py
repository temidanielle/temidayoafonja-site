# -*- coding: utf-8 -*-
"""Short-form documents: the combined bank, and one page per candidate."""
import os
...
def bank(n, path, stamp):
    ...
    callout(d, "Every spoken line below is a whole paragraph of the approved "
               "long-form script, in the script's own words. No Short "
               "introduces research, wording or a claim the video does not "
               "already make.")
    rows = []
    for r in SH.rows(n):
        rows.append([r["key"].split("_", 2)[2].replace("_", " "),
                     r["hook"], format(r["words"], ","), "0:%02d" % r["secs"]])
    table(d, ["Candidate", "Stop scroll", "Words", "At 165 wpm"], rows,
...
def single(n, r, path, stamp):
    ...
    kv(d, "Generated", stamp)
    _one(d, n, r)
    footer_note(d, "Vertical. No burned-in caption decision is made here; "
                   "Shorts captions are set at upload.")
    d.save(path)
    return path
