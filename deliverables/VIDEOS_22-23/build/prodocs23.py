# -*- coding: utf-8 -*-
"""The production documents an editor works from.

Run of show, the camera and full-screen map, the motion and reveal map, the
sound cue map, the asset index and the B-roll notes. Nothing here restates
the script; each document answers one question the edit will actually ask.
"""
import os
...
def run_of_show(n, path, stamp):
    ...
    para(d, M.thumbnail(n), size=13, bold=True, after=12)
    caption(d, "Both are exact. Thumbnail artwork is approved separately and "
               "is not part of this package.")
    callout(d, "The markers below come from the script and are navigation "
               "only. Chapters are cut after the final edit, from real "
               "export timing.")
    rows = []
    for mk, label, ps in M.sections(n):
        keys = [f["key"] for f in F.SETS[n]
                if any(M._norm(f["trigger"]) == M._norm(p) for p in ps)]
        rows.append([mk, label, format(sum(len(p.split()) for p in ps), ","),
                     "%d" % len(ps),
                     ", ".join(k.split("_", 2)[2].replace("_", " ").lower()
                               for k in keys) or "camera throughout"])
    table(d, ["Marker", "Section", "Words", "Paragraphs", "Full-screen cues"],
          rows, widths=[0.6, 1.9, 0.6, 0.8, 2.8], size=8.5)

    h(d, "Editing restraint")
    table(d, ["Element", "Target", "Note"],
          [[a, b, c] for a, b, c in EDIT], widths=[1.5, 1.7, 3.5], size=8.5)
    ...
    bullets(d, [
      ...
      "The two may be visually related but must not become cluttered.",
...
def camera_map(n, path):
    ...
    L += ["  %d camera stretches" % cam,
          "  %d full-screen cues, one per card" % full, "", hr(), ""]
    for kind, *rest in SP.spine(n):
        if kind == "SECTION":
            ...
            for p in ps:
                L.append("      %s" % (p[:96] + ("..." if len(p) > 96 else "")))
            L.append("")
        else:
            f, p = rest
            L.append("  [ FULL SCREEN ]  %s" % f["key"])
            L.append("      TRIGGER:  %s" % f["trigger"])
            L.append("      STATES:   %s"
...
def motion_map(n, path):
    ...
    for f in F.SETS[n]:
        ...
        for i, s in enumerate(f["states"], 1):
            L.append("    %d. %-40s %s" % (i, s["name"], s["reveal"]))
        L += ["    HOLD:       %s" % f["hold"], ""]
    L += [hr(), "",
          "STANDING RULES", "",
...
def sound_map(n, path):
    ...
    for f in F.SETS[n]:
        if f["sound"].lower().startswith("no accent"):
            continue
        total += f["sound"].lower().count("accent")
        L += ["%s" % f["key"], "    %s" % f["sound"], ""]
    L += [hr(), "",
          "SILENT BY DESIGN", ""]
    for f in F.SETS[n]:
...
def asset_index(n, path, files, extra=None):
    ...
    L += [M.title(n), "",
          "%d cards, %d image states, plus %d Watch Next candidate cards."
          % (len(F.SETS[n]), len(F.states(n)), len(extra or [])),
...
BROLL = {
 22: [("Reading on a laptop, over the shoulder, no screen legible",
      ...
      ("Hands and a notebook, writing four short words",
       "Under the method recap, if the cut needs a breath before the first "
       "example.",
...
def broll(n, path):
    ...
    L += [M.title(n), "",
          ...
          "needs them. Substantive B-roll is full screen. Nothing here is",
          "required for the video to work.",
          "", hr(), ""]
