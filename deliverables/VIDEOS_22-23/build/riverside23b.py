# -*- coding: utf-8 -*-
"""The Riverside / Co-Creator master editing prompt, rebuilt on the new
script.

Nothing here says "add a graphic". Every cue carries its exact trigger
sentence, its mode, what the visual is for, its layout, its reveal order and
emphasis state, how long to hold, whether it takes a sound accent, its asset
filename and its source note.
"""
...
STANDING = [
 ("Camera-led",
  ...
  "These scripts are shorter than the previous drafts and carry fewer cues "
  "on purpose."),
 ("True full screen",
 ...
 ("Retired assets",
  "Cards the previous package carried are listed at the end of this prompt. "
  "They have no cue in this script and must not be cut back in."),
]
...
def prompt(n, path, stamp):
    ...
    L += [M.title(n),
          ...
          "Generated %s" % stamp, "",
          "%s spoken words by whitespace count. %d camera stretches against "
          "%d" % (format(M.word_count(n), ","), cam, full),
          "full-screen cues, and %d of the spoken words are delivered on "
          "camera." % on_cam,
          "Runtime is observed at Temidayo's delivery pace. This script",
          "carries no timing markers, so nothing in it can be mistaken for a",
          "runtime, a chapter or an SRT time.",
          "", hr(), "", "HOW TO USE THIS", "",
    ...
    for kind, *rest in SP.spine(n):
        if kind == "SECTION":
            ...
            L.append("")
    L += [hr(), "", "RETIRED. DO NOT CUT THESE BACK IN.", ""]
