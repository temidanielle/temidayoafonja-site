# -*- coding: utf-8 -*-
"""Build the two Video 7 script files from the production package.
...
clean TXT cannot drift apart.
  ...
  Video_7_Recording_Script_Clean.txt
...
def build():
    ...
    para("How to Show Your Impact at Work When You Built It From Scratch",
         size=24, bold=True, color=NAVY, after=2, spacing=1.1)
    para("Video 7  ·  Teleprompter script with slide markers", size=12,
    ...
    out_docx = os.path.join(
        HERE, "Video_7_Teleprompter_Script_with_Slide_Markers.docx")
    doc.save(out_docx)

    # ---------------------------------------------------- clean spoken script
    spoken = [b.strip() for b in sec
              if not HDR.match(b.strip()) and not b.strip().startswith("Target")]
    out_txt = os.path.join(HERE, "Video_7_Recording_Script_Clean.txt")
