...
class Field(Flowable):
    """A single PRINTABLE write-in box drawn at flow position (v1.0.2).

    v1.0.1 drew an interactive AcroForm text field here. v1.0.2 is print-first:
    this draws a clean bordered box with faint ruled lines for handwriting,
    at the identical position and size, so every page layout, page count, and
    the surrounding design are unchanged. No interactive form fields are
    emitted, so the PDF carries no AcroForm at all."""
    _seen = set()
    ...
    def drawOn(self, canvas, x, y, _sW=0):
        # Match the absolute lower-left the AcroForm field used, so the printable
        # box lands in exactly the same place whether or not it is nested in a
        # Table (which translates the canvas).
        try:
        ...
        bx, by = ax, ay + 2
        canvas.saveState()
        # the write-in box: white ground, brand hairline border (same geometry)
        canvas.setFillColor(FIELDBG); canvas.setStrokeColor(self.border)
        canvas.setLineWidth(0.9)
        canvas.rect(bx, by, self.width, self.height, fill=1, stroke=1)
        # ruled writing lines inside taller (multiline) boxes
        if self.multiline and self.height >= 34:
            canvas.setStrokeColor(HAIR); canvas.setLineWidth(0.5)
            gap = 18.0
            yy = by + self.height - gap
            while yy > by + 5:
                canvas.line(bx + 5, yy, bx + self.width - 5, yy)
                yy -= gap
        canvas.restoreState()
...
class KTPDoc(BaseDocTemplate):
    def __init__(self, filename, footer_title="Keep the Proof", url="temidayoafonja.com", version=None, **kw):
        super().__init__(filename, pagesize=letter, leftMargin=MARGIN, rightMargin=MARGIN,
                         topMargin=MARGIN, bottomMargin=MARGIN, **kw)
        self.footer_title = footer_title; self.url = url; self.version = version
    ...
    def _footer(self, canv, doc, light=False):
        ...
        left = self.footer_title if not self.version else f"{self.footer_title}  ·  {self.version}"
        canv.drawString(MARGIN, 40, left)
