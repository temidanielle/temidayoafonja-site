...
def story(S, P, LEAD, H2, H3, EY, KI, NOTE, SP, bullets, CO, TBL, RULE,
    ...
    st += [SP(6), EY("Capturing well"),
        ...
        H3("Assigned responsibility is only the starting point"),
        P("Your job description tells a reader what you were supposed to do. It rarely tells them what you did. The gap between the two is where most of your real contribution lives. Record the assignment, then record what you drove beyond it."),
    ...
    st += [SP(6), EY("The hard cases"),
        ...
        RULE(),
        P("A great deal of valuable work produces no tidy number. Coordination, judgment, care, and prevention rarely come with a percentage attached. You can still record this work. Describe the change in credible, concrete terms, and never force a figure you do not have."),
        ...
        P("Specific improvement is what persuades a reader. Compare &#8220;made things better&#8221; with &#8220;the weekly report that used to take two people a full day now takes one person an hour.&#8221; A concrete, honest particular is more persuasive than a round number you cannot defend."),
        SP(2),
    ...
    st += [SP(6), EY("The invisible work"),
        ...
        SP(2),
        example_card("Prevention, captured with care  ·  Theo R., security analyst",
    ...
    st += [Bookmark("From internal language to portable language", 1), IconMark(chip_mark(ic_translate_arrow)), SP(6), EY("Translate"),
        ...
        RULE(),
        P("A record only your current employer can read is a record that expires when you leave. Translation is the skill of saying the same true thing in words an outsider can follow. It keeps the meaning and changes only the words, turning a private shorthand into a description that travels."),
    ...
    st += [SP(4), H3("Built in front of you"),
        ...
        CO("Maya&#8217;s Proof Line",
           ...
           bg="sand", bar=GOLD),
        SP(4),
        NOTE("A Proof Line does not need a number to be strong, and it must never contain an invented one. It also never claims sole credit for a shared result. If a line would only sound impressive by overstating your ownership, it is wrong, and you rewrite it until it is true."),
    ]
    st += [PB()]
    ...
    st += [SP(6), EY("Full sequence, end to end"),
        ...
        example_card("2 · Full Entry  ·  the fields that matter here",
            ...
            [("Formal vs. actual", "Assigned to routine access reviews; drove a correction and a process change."),
    ...
    st += [Bookmark("The complete 60-minute setup", 1), IconMark(chip_mark(ic_clock60)), SP(6), EY("Start here"),
        ...
        RULE(),
        P("<i>With focused time, this takes about an hour. In shorter sittings, work one step at a time and stop wherever you need to.</i>"),
        P("This is the first hour. Follow it in order and you will finish with a working system and your first real entries. The promise here is modest and real: you can leave the hour with the system running and something true already in it. Reconstructing a whole career takes longer, and this hour does not pretend otherwise."),
        TBL([["Time", "What you do"],
             ["0&#8211;10 min", "Read the permission rule and the three tiers again. Choose a private home you control and turn on a password and multifactor authentication. Permission comes first, then storage.<br/><br/><font name=\"DM-Bold\" size=\"7.5\" color=\"#C1440E\">GOOD PLACE TO PAUSE</font>"],
             ...
             ["20&#8211;40 min", "Write at least three Quick Captures from that sweep. Rough is fine. Speed beats polish.<br/><br/><font name=\"DM-Bold\" size=\"7.5\" color=\"#C1440E\">GOOD PLACE TO PAUSE</font>"],
             ["40&#8211;50 min", "Expand your strongest capture into a Full Entry. Fill only the fields that apply.<br/><br/><font name=\"DM-Bold\" size=\"7.5\" color=\"#C1440E\">GOOD PLACE TO PAUSE</font>"],
    ...
    # 27b. MATCH YOUR PROOF TO A ROLE (printable one-page tool) — v1.0.2
    st += [PB(), SP(6), EY("Use it"),
        H2("Match Your Proof to a Role"), RULE(),
    ...
    _mhead = ["What the role asks for", "My Proof Line that shows it", "The gap I will name"]
    _mrows = [[Paragraph(h, S["tbl_h"]) for h in _mhead]]
    _mrows.append([
    ...
    _mrows += [[Paragraph("&nbsp;", S["tbl"]) for _ in range(3)] for _ in range(3)]
    _mt = Table(_mrows, colWidths=[CONTENT_W*0.32, CONTENT_W*0.42, CONTENT_W*0.26],
                rowHeights=[None, None, 66, 66, 66], repeatRows=1)
    _mt.setStyle(TableStyle([("VALIGN",(0,0),(-1,-1),"TOP"),
    ...
    # 28. STARTING LATE: WHEN YOUR ACCESS IS ALREADY GONE (module) — v1.0.2
    st += [Bookmark("Starting Late: When Your Access Is Already Gone", 1), SP(6), EY("Starting late"),
        H2("Starting Late: When Your Access Is Already Gone"), RULE(),
        P("You may be reading this after a reduction, a reorganization, or a role that has ended. The systems that held your work are closed, and the record you wish you had does not exist yet. That is a hard place to begin, and a common one. You can still rebuild a useful, honest account of your work from what is still yours. This module shows you how, at a steady pace, and without pretending the result will be complete."),
        P("Take it in short sittings. The aim is to rebuild what you can stand behind, mark clearly what you cannot, and then start capturing forward so you never have to do this again."),
        ...
        SP(4),
        CO("The same rule, at a harder moment",
           "Losing access does not loosen the permission rule. You still record only your own recollection and what you are permitted to keep. Where something is unclear, you leave it out and, if it matters, you ask someone qualified.",
           bg="navy"),
    ]
    st += [PB()]

    st += [SP(6), EY("Starting late, continued"),
        H2("Reconstruct by Period"), RULE(),
        P("Work role by role, or year by year. Take each period in turn and recall its major projects, the decisions you made, the problems you caught, and the feedback you remember. Fill one row per role or year. Print this page, or copy the columns into any document you control, and add rows as you need them."),
        SP(6),
    ]
    _rhead = ["Role or year", "Major projects", "Decisions I made", "Problems I caught", "Feedback I remember"]
    _rrows = [[Paragraph(h, S["tbl_h"]) for h in _rhead]]
    _rrows += [[Paragraph("&nbsp;", S["tbl"]) for _ in range(5)] for _ in range(5)]
    _wt = Table(_rrows, colWidths=[CONTENT_W*0.14, CONTENT_W*0.225, CONTENT_W*0.215,
                CONTENT_W*0.21, CONTENT_W*0.21], rowHeights=[None]+[92]*5, repeatRows=1)
    _wt.setStyle(TableStyle([("VALIGN",(0,0),(-1,-1),"TOP"),("BACKGROUND",(0,0),(-1,0),NAVY),
    ...
    st += [SP(6), EY("Starting late, continued"),
        H2("Anchors and confidence"), RULE(),
        H3("Memory anchors"),
        P("If a period feels blank, start from a specific moment rather than a summary. These anchors tend to pull the rest of the memory back:"),
    ]
    st += bullets([
        "A hard call you had to make.",
        "A launch, a delivery, or a deadline you carried.",
        "A problem you caught early, before it spread.",
        "A piece of feedback you received, good or corrective.",
        "A moment someone relied on you to get something right.",
    ])
    st += [SP(4), H3("Mark your confidence on every reconstructed entry"),
        P("Memory is uneven, and an honest record says so. Mark each reconstructed entry, or each field, with one of three levels:"),
    ]
    st += bullets([
        "<b>Sure.</b> You remember this clearly and could stand behind it if asked.",
        "<b>Approximate.</b> The shape is right, but a detail such as a date or a number is your best estimate.",
        "<b>Unsure.</b> You think it happened this way, but you would want to confirm it before relying on it.",
    ])
    st += [SP(4),
        CO("Approximate and Unsure entries wait",
           "An entry marked Approximate or Unsure does not become a Proof Line until you confirm it. Keep it in the record, keep the marker on it, and promote it only when you can move it to Sure.",
           bg="sand", bar=GOLD),
    ]
    st += [PB()]

    st += [SP(6), EY("Starting late, continued"),
        H2("The 60-minute starting-late setup"), RULE(),
        P("<i>With focused time, this takes about an hour. In shorter sittings, work one step at a time and stop wherever you need to.</i>"),
        P("This mirrors the sixty-minute setup earlier in the guide, adjusted for rebuilding from memory. Follow it in order. You will finish with a working system and your first reconstructed entries, each one marked for confidence."),
        TBL([["Time", "What you do"],
             ["0&#8211;10 min", "Read the permission rule and the three tiers again. Choose a private home you control and turn on a password and multifactor authentication. Permission comes first, then storage.<br/><br/><font name=\"DM-Bold\" size=\"7.5\" color=\"#C1440E\">GOOD PLACE TO PAUSE</font>"],
             ["10&#8211;25 min", "Gather what is still yours to use: your own memory, publicly disclosed outcomes, and anything you were expressly permitted to keep. Pick one role or year you remember well."],
             ["25&#8211;45 min", "Write at least three Quick Captures from memory, anchored to moments you recall clearly. Mark each one Sure, Approximate, or Unsure.<br/><br/><font name=\"DM-Bold\" size=\"7.5\" color=\"#C1440E\">GOOD PLACE TO PAUSE</font>"],
             ["45&#8211;55 min", "Expand your strongest capture into a Full Entry. Keep the confidence marker on each field.<br/><br/><font name=\"DM-Bold\" size=\"7.5\" color=\"#C1440E\">GOOD PLACE TO PAUSE</font>"],
             ["55&#8211;60 min", "Translate one entry you have marked Sure into a portable Proof Line. Leave Approximate and Unsure entries out of your Proof Lines until you can confirm them. Set a date to begin capturing in real time."]],
            [CONTENT_W*0.16, CONTENT_W*0.84]),
        SP(4),
        CO("What you have at minute sixty",
           "A private system you control, three or more reconstructed captures each marked for confidence, one full entry, and one Proof Line built only from what you are sure of. From here you capture forward, so you never have to reconstruct again.",
           bg="navy"),
    ]
    st += [PB()]
    ...
    st += [SP(6), EY("Fill it in"),
        ...
        RULE(),
        P("Use the printable pages that follow to begin now. The standalone Career Evidence Ledger, included with your purchase, repeats these same forms so you can reuse them as often as you need. Print the pages you need, or copy the prompts into any notebook or document you control."),
        SP(6),
    ...
    st += [Bookmark("About Temidayo Afonja", 0), SP(6), EY("About the author"),
        ...
        P("Temidayo Afonja is the Founder of The Density Group. For eighteen years, Temidayo has worked across Big Four consulting, life sciences, and technology, close to where talent decisions get made, at the intersection of workforce strategy, organizational design, and how careers form."),
