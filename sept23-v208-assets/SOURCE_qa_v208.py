...
# ── L. the report's SOP-preview paragraph ──────────────────────────────────
# These two rows are embedded in the very document they check, so the report
# builder is run twice: the first pass writes the corrected file, the second
# reads it back and embeds the result. Nothing is asserted about a file that
# does not exist yet.
REPORT = (f"{OUT}/Free_Flagship_60MIN_v2.0.11_Change_Log_and_QA_Report.docx")
RPT = flat(doc_text(REPORT)) if os.path.exists(REPORT) else ""
_sop_pdf = pymupdf.open(SOP_PDF)
_orient = ["L" if p.rect.width > p.rect.height else "P" for p in _sop_pdf]
_last = [l for l in _sop_pdf[-1].get_text().split("\n")
         if l.strip() and "FREE FLAGSHIP SOP" not in l]
chk("L", "The report names the shipped v2.0.10 preview and its true orientation",
    "PREVIEW_SOP_60MIN_v2.0.10_LibreOffice.pdf" in RPT
    and "pages 4 through 8 portrait" in RPT
    and "The v2.0.9 export is EXACTLY EIGHT PAGES" not in RPT
    and "pages 4 through 7 portrait" not in RPT,
    f"the shipped preview is {len(_orient)} pages, {''.join(_orient)}: page 1 "
    f"portrait, pages 2-3 landscape, pages 4-8 portrait")
chk("L", "The pagination claim is scoped to the shipped preview, not to DOCX",
    "NOT ABOUT DOCX PAGINATION IN GENERAL" in RPT
    and "renderers break pages differently" in RPT
    and "not edited to force a particular pagination" in RPT.lower()
    and len(_last) >= 12,
    f"page {len(_orient)} carries {len(_last)} substantive lines, so it is not a "
    f"blank or stub, and the claim is about this export rather than about how any "
    f"other renderer would paginate the same .docx")
