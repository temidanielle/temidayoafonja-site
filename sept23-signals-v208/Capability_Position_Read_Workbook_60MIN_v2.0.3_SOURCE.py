# -*- coding: utf-8 -*-
"""Capability Position Read — Workbook, 60-minute edition, v2.0.3 CANDIDATE.

CHANGE FROM v2.0.2: two of the seven move-category names on page 8 are matched
to the approved carousel wording. The old "Repair formation conditions" is now
"Repair the conditions", and the old "Seek an external perspective" is now
"Seek an outside perspective". The other five already matched the carousel
exactly and were not touched, and all seven descriptions are unchanged. The deck
carries the same two renames, so the workbook and the slide agree.

CHANGE CARRIED OVER FROM v2.0.1: page 8, the Next-Move Note, gains
one line under WHAT HAPPENS NEXT:

    Two signals I will watch before my rescore date: [signal_1] and [signal_2]

It is inside the WHAT HAPPENS NEXT block, which grows from 64pt to 86pt to hold
it, so the line belongs to that prompt rather than floating between sections.
Two new text fields are added, signal_1 and signal_2, taking the field count
from 68 to 70. Every pre-existing field keeps its name, type and behaviour;
only rescore_date moves, 22pt down, because it sits below the grown block.

The pacing cues in PACE are also re-cut, because the deck's timeline changed to
make room for the Signals Worth Watching section. That is a cross-reference
fix, not an editorial one: a workbook that tells a participant to be on page 7
at minute 31 while the deck is on page 7 at minute 30 is simply wrong. Pages 2
and 3 keep their cues because nothing before minute 16 moved.

v2.0.2 HAS NOT BEEN BEHAVIOUR-TESTED IN ACROBAT. v2.0.1's test status does not
carry over to it and it must be retested before participant distribution.
...
OUT = "v2/out/Capability_Position_Read_Workbook_60MIN_v2.0.3_CANDIDATE.pdf"
...
MOVES = [
 ...
 ("Repair the conditions", "Change the work, not the employer, first."),
 ("Prepare for exit", "Your evidence gives you reason to prepare in case the conditions do not repair."),
 ("Seek an outside perspective", "High stakes, incomplete evidence, or a boundary position.")]

c = canvas.Canvas(OUT, pagesize=letter)
c.setTitle("Capability Position Read — Workbook (60-minute edition)")
c.setAuthor("Temidayo Afonja")
c.setSubject("Participant workbook. Private to the participant; nothing is collected.")
form = c.acroForm
...
PHASES = {2: ("PHASE 1", "INITIAL READ"), 3: ("PHASE 1", "INITIAL READ"),
          4: ("PHASE 2", "EVIDENCE STANDARD"),
          5: ("PHASE 3", "CORRECTED READ"), 6: ("PHASE 3", "CORRECTED READ"),
          7: ("PHASE 3", "CORRECTED READ"), 8: ("PHASE 4", "NEXT-MOVE NOTE")}
# Pacing cues follow the deck's re-cut timeline. The Signals Worth Watching
# section takes three minutes out of calibration, the re-total, the sensitivity
# check and the close buffer, so every phase from the evidence standard onward
# starts and ends earlier, except the Next-Move Note, which the new section
# pushes one minute later. Pages 2 and 3 are untouched because nothing before
# minute 16 moved. The twelve-minute evidence block is shifted, never shortened.
PACE = {2: "Live: minutes 5–9", 3: "Live: minutes 9–14",
        4: "Live: minutes 14–18", 5: "Live: minutes 18–30", 6: "Live: minutes 18–30",
        7: "Live: minutes 30–33", 8: "Live: minutes 42–46"}

def page_frame(n, eyebrow_txt):
    """running header + phase band + pacing cue + footer"""
    tracked("CAPABILITY POSITION READ", ML, PH - 34, 6.8, LIGHT, "Mont-B", 1.3)
    c.setFont("Mont-B", 6.8); c.setFillColor(LIGHT)
    c.drawRightString(PW - MR, PH - 34, "WORKBOOK")
    c.setStrokeColor(HAIR); c.setLineWidth(0.5); c.line(ML, PH - 44, PW - MR, PH - 44)
    if n in PHASES:
        num, nm = PHASES[n]
        bw = 150
        rect(ML, PH - 62, bw, 15, fill=NAVY)
        c.setFont("Mont-B", 6.6); c.setFillColor(GOLD)
        c.drawString(ML + 8, PH - 57.5, num)
        c.setFillColor(CREAM)
        c.drawString(ML + 8 + 34, PH - 57.5, nm)
    if n in PACE:
        c.setFont("DMSans-I", 7.6); c.setFillColor(MUTED)
        c.drawRightString(PW - MR, PH - 57.5, PACE[n] + "   ·   Replay: your own pace")
    ...
    if eyebrow_txt:
        tracked(eyebrow_txt, ML, PH - 80, 8.2, GOLD, "Mont-B", 1.4)

def head(title_txt, kicker_txt, y=PH - 106, size=25):
...
c.drawString(ML + 24, PH - 149, "Workbook")
rect(ML + 24, PH - 168, 92, 1.6, fill=GOLD)
c.setFont("Cormorant-I", 15.5); c.setFillColor(CREAM)
for i, ln in enumerate(wrap("This workbook helps you read your current position. "
...
BLOCKS = [
 ...
 ("PRIVACY",
  "Nothing in this workbook is submitted, collected or shared. No score, state, employer or "
  "identifying information is requested at any point. This workbook is yours."),
 ("KEEP BOTH READINGS",
...
y -= 4
# how to use it — the live session and the replay are different experiences
hh = 44
rect(ML, y - hh, CW / 2 - 6, hh, fill=NAVY)
tracked("IN THE LIVE SESSION", ML + 16, y - 15, 7, GOLD, "Mont-B", 1.2)
c.setFont("DMSans", 9); c.setFillColor(CREAM)
c.drawString(ML + 16, y - 29, "Follow the presenter's pace. Every page shows")
c.drawString(ML + 16, y - 39, "the minutes it belongs to.")
rect(ML + CW / 2 + 6, y - hh, CW / 2 - 6, hh, fill=PALE)
tracked("ON THE REPLAY", ML + CW / 2 + 22, y - 15, 7, NAVY, "Mont-B", 1.2)
c.setFont("DMSans", 9); c.setFillColor(NAVY)
c.drawString(ML + CW / 2 + 22, y - 29, "Pause and work at your own pace. Nothing")
c.drawString(ML + CW / 2 + 22, y - 39, "here depends on keeping up.")
y -= hh + 14

tracked("FOUR PHASES, EIGHT PAGES", ML, y, 7.2, MUTED, "Mont-B", 1.3); y -= 17
PH4 = [("PHASE 1", "PAGES 2–3", "Initial Read",
        "Twelve statements from memory, two totals, a first provisional placement."),
       ("PHASE 2", "PAGE 4", "Evidence Standard",
        "What a corrected score has to survive, and what a question mark means."),
       ("PHASE 3", "PAGES 5–7", "Corrected Read",
        "All twelve again with evidence, corrected totals, sensitivity, corrected position."),
       ("PHASE 4", "PAGE 8", "Next-Move Note",
        "Three lines and a rescore date. The artifact you leave with.")]
for num, pages, nm, desc in PH4:
    rect(ML, y - 26, 66, 26, fill=NAVY)
    c.setFont("Mont-B", 6.6); c.setFillColor(GOLD)
    c.drawCentredString(ML + 33, y - 11, num)
    c.setFont("Mont-B", 6.0); c.setFillColor(LIGHT)
    c.drawCentredString(ML + 33, y - 20.5, pages)
    rect(ML + 72, y - 26, CW - 72, 26, fill=PALE)
    c.setFont("DMSans-B", 9.6); c.setFillColor(NAVY)
    c.drawString(ML + 86, y - 11, nm)
    c.setFont("DMSans", 8.4); c.setFillColor(MUTED)
    c.drawString(ML + 86, y - 21, desc)
    y -= 30
y = para("Everything is available from the start. Nothing is released in stages.",
         ML, y - 12, CW, "DMSans-I", 9, MUTED)
guard(y, 1)
c.showPage()
...
def statement_block(items, first_n, prefix, y, evidence=False):
    ...
    for i, t in enumerate(items):
        ...
        if evidence:
            c.setFont("DMSans", 7.2)
            c.drawString(fx + 40, y - 43, "with one ?")
            c.drawString(fx + 40, y - 51, "if unsupported")
            ey = y - body_h - 30
...
# The "A 3 means mixed / a question mark means unsupported" band was removed in
# v2.0: the A 3 and A ? rows above already carry both halves of that sentence,
# and restating it made the page read as if it were two different rules.
y -= 12
...
y -= 6
bh = 14 + para_h("An axis total of 19 to 30 reads high and 6 to 18 reads low. A total of 17 to "
                 "21 falls inside the boundary band, and the boundary reading OVERRIDES that "
                 "split: no single state is assigned and you read both neighbouring states. "
                 "The arithmetic does not change — only what the total entitles you to conclude.",
                 "DMSans", 9.4, CW - 124)
rect(ML, y - bh, CW, bh, fill=PALE)
rect(ML, y - bh, 3, bh, fill=RUST)
tracked("THE 17–21 BAND", ML + 16, y - 15, 7, NAVY, "Mont-B", 1.2)
para("An axis total of 19 to 30 reads high and 6 to 18 reads low. A total of 17 to 21 falls "
     "inside the boundary band, and the boundary reading OVERRIDES that split: no single state "
     "is assigned and you read both neighbouring states. The arithmetic does not change — only "
     "what the total entitles you to conclude.",
     ML + 112, y - 15, CW - 124, "DMSans", 9.4, NAVY)
y -= bh + 8
dh = 14 + para_h("If exactly ONE axis is inside the band, read the two adjacent states that "
                 "the OTHER axis selects. If BOTH axes are inside the band, record Boundary "
                 "and assign no single state.", "DMSans", 9.4, CW - 124)
rect(ML, y - dh, CW, dh, fill=PALE)
rect(ML, y - dh, 3, dh, fill=RUST)
tracked("ONE AXIS, OR BOTH", ML + 16, y - 15, 7, NAVY, "Mont-B", 1.2)
para("If exactly ONE axis is inside the band, read the two adjacent states that the OTHER axis "
     "selects. If BOTH axes are inside the band, record Boundary and assign no single state.",
     ML + 112, y - 15, CW - 124, "DMSans", 9.4, NAVY)
y -= dh + 10
y = para("Every neighbouring score stays inside the 1 to 5 scale — a 1? is never tested at 0, a "
         "5? is never tested at 6. This is an operational convention, not a statistical "
         "confidence interval, and it carries no margin of error.",
         ML, y, CW, "DMSans-I", 9, MUTED) - 22

y = para("An evidence line is a phrase, not an essay. One instance, one counterexample, or one "
         "honest \u201cno qualifying instance in the window\u201d is enough. Twelve statements "
         "in twelve minutes is sixty seconds each.",
         ML, y, CW, "DMSans-I", 9, MUTED) - 10
guard(y, 4)
c.showPage()
...
GUT, CELLH = 58, 92
cw2 = (CW - GUT) / 2.0
# Compounding is set apart because it is the one square where BOTH axes are
# high — a structural fact about the matrix, not a ranking. Navy rather than
# rust: rust is this system's caution accent (the 3+ INCOMPLETE chip), and on
# Compounding it would read as a warning. "State is not identity" stays on the
# page beneath, so nothing here implies a square to aim for.
QUAD = [("DEPTH TRAP", "Deep expertise fused to one context.", 0, 0, False),
        ("COMPOUNDING", "Deep capability that travels.", 1, 0, True),
        ("STAGNANT", "The work stopped asking more of you.", 0, 1, False),
        ("FRAGILE", "Options on paper, not enough depth beneath.", 1, 1, False)]
mtop = y
for k, v, col_i, row_i, lead in QUAD:
    cx = ML + GUT + col_i * cw2
    cy = mtop - (row_i + 1) * CELLH
    rect(cx, cy, cw2, CELLH, fill=NAVY if lead else PALE, stroke=LIGHT, lw=0.7)
    checkbox(f"state_{k.lower().replace(' ', '_')}", cx + 14, cy + CELLH - 30, 13)
    c.setFont("Mont-B", 9); c.setFillColor(GOLD if lead else NAVY)
    c.drawString(cx + 34, cy + CELLH - 27, k)
    para(v, cx + 34, cy + CELLH - 47, cw2 - 48, "DMSans", 9, LIGHT if lead else MUTED)
    if lead:
        c.setFont("DMSans-I", 7.6); c.setFillColor(LIGHT)
        c.drawString(cx + 34, cy + 12, "The one square where both axes are high.")
# axis labels
...
SIGNAL_LINE = "Two signals I will watch before my rescore date:"
for k, q, nm, hint in PROMPTS:
    # WHAT HAPPENS NEXT carries the signals line, so its block is 22pt taller
    ph = 86 if nm == "nmn_next" else 64
    ...
    if nm == "nmn_next":
        c.setFont("DMSans", 9.4); c.setFillColor(NAVY)
        c.drawString(ML + 17, y - 79, SIGNAL_LINE)
        x1 = ML + 17 + c.stringWidth(SIGNAL_LINE, "DMSans", 9.4) + 10
        right = ML + CW - 17
        aw = c.stringWidth("and", "DMSans", 9.4)
        fw = (right - x1 - aw - 20) / 2.0      # two blanks, one "and", even gaps
        assert fw >= 90, f"signals blanks too narrow: {fw:.1f}pt"
        txtfield("signal_1", x1, y - 84, fw, 16, size=9.5, kind="text")
        c.setFont("DMSans", 9.4); c.setFillColor(NAVY)
        c.drawString(x1 + fw + 10, y - 79, "and")
        txtfield("signal_2", x1 + fw + aw + 20, y - 84, fw, 16, size=9.5, kind="text")
    y -= ph + 11
...
def attach_field_behaviour():
    ...
    SRC = "v2/out/Capability_Position_Read_Workbook_60MIN_v2.0.3_CANDIDATE.pdf"
    ...
    VAL_15Q = ('if(event.value != "" && !' + CORR_RE + '.test(event.value)) {\n'
               '  app.alert("Enter a score from 1 to 5. You may add one question mark to any '
               'defensible score from 1 to 5, for example 2?, 3?, or 4?, when the evidence '
               'cannot support the number either way. A question mark on its own is not a '
               'score.");\n'
               '  event.rc = false;\n}')
    ...
    for page in w.pages:
        for a in page.get("/Annots", []):
            ...
            if INIT15.match(nm):
                ...
                # able to type the total in, without printing the workbook.
