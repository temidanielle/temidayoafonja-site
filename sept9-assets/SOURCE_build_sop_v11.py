# -*- coding: utf-8 -*-
"""September 9, 2026 facilitator / delivery SOP — v1.1 FINAL.

v1.1 closes the one open item v1.0 flagged: the recording policy, which the
source family did not establish and which v1.0 therefore declined to invent. The
owner has now decided it, and section 4 states it as a rule rather than a
question. Nothing else in the document changes, and no teaching content is
...
DST = f"{OUT}/Capability_Formation_Career_Stalling_SOP_Sept9_2026_v1.1_FINAL.docx"
...
CHECKLIST = [
    ...
    "v3.5.1_FINAL.pptx. Downloaded locally, not streamed from a share.",
...
RECORDING = [
    ...
    "recorded portion. Start the recording on slide 1.",
    "Deliver the recording and privacy disclosure inside the first five minutes — it "
    "sits on slide 3 at 3:00–4:30, alongside the boundary the session already sets "
    "out loud.",
    "AT 35:00, STOP THE RECORDING BEFORE TAKING PARTICIPANT QUESTIONS. Stop it; do not "
    "pause it. Look at the screen and confirm it has actually stopped before the first "
    "participant-specific question.",
    "Say out loud that the recording has stopped, then advance to slide 15.",
    "Minutes 35:00–45:00 are live-only Q&A and are NOT included in the distributed "
    "replay, under any distribution model.",
    "Do not read personal participant situations, employer names or sensitive chat "
    "messages into the recorded teaching.",
    "If the platform captures the whole meeting automatically, retain the raw file "
    "privately and trim the distributed replay so it ends at minute 35.",
    "The replay contains the teaching and recognition lesson only — never participant "
    "Q&A.",
]
...
def build():
    ...
    para(body_p,
         ...
         "The run of show below is read out of the v3.5.1 speaker notes at build time, "
    ...
    para(body_p,
         "Taken from the v3.5.1 speaker notes. Fifteen live slides run 0:00 to 45:00 "
    ...
    para(sub_p, "LOCKED POLICY — record 0:00–35:00 only")
    para(body_p,
         "Decided by the owner and locked for this session. v1.0 of this SOP left it "
         "open because the source family established no policy; it is now a rule. This "
         "is a September 9 policy: the 60-minute flagship records minutes 0 to 50, and "
         "that timing belongs to a different architecture and does not carry across.")
    for line in RECORDING:
        para(bullet_p, line)
    para(sub_p, "Participant privacy — unchanged")
    ...
    para(bullet_p,
         "This SOP is v1.1 FINAL and governs deck v3.5.1 FINAL. v1.0 was the same "
         "document with the recording policy still open; v1.1 closes it. It is a new "
         "document, "
         "not a revision of Facilitator Guide v3.2.")
    para(bullet_p,
         "Source of truth for delivery: the v3.5.1 deck and its speaker notes. This SOP "
    ...
    para(bullet_p,
         "No open items. The recording policy, which v1.0 left open because the source "
         "family did not establish one, is locked in section 4 and mirrored in the "
         "v3.5.1 speaker notes on slides 1, 3, 14 and 15.")
    para(foot_p,
         "CAREER STALLING SOP v1.1 FINAL  ·  SEPTEMBER 9, 2026  ·  INTERNAL ONLY  ·  "
         "Prepared " + STAMP)
...
def qa(path, pdf=None):
    ...
    flat = re.sub(r"\s+", " ", T)
    chk("The recording policy is locked and stated as a rule",
        "RECORD MINUTES 0:00–35:00 ONLY" in flat
        and "STOP THE RECORDING BEFORE TAKING PARTICIPANT QUESTIONS" in flat
        and "Stop it; do not pause it." in flat
        and "confirm it has actually stopped" in flat
        and "are NOT included in the distributed replay" in flat,
        "the open item v1.0 flagged is closed")
    chk("The September 23 flagship's 0-to-50 timing is not copied across",
        "0 to 50" in flat and "does not carry across" in flat
        and "0:00–50" not in flat and "minute 50" not in flat,
        "named once, only to say it does not apply here")
    chk("The recorded portion matches the deck's own teaching block",
        SPAN[15] == ("35:00", "45:00") and "0:00–35:00 ONLY" in flat,
        "the recording boundary and the Q&A boundary are the same instant")
